...
import type { OrgDepartment, OrgDesignation } from '../types';
...
export default function DesignationManagement() {
  ...
  const loadDesignations = useCallback(() => {
    ...
    setLoading(true);
    setError(null);
    organizationApi.listDesignations(Number(departmentId))
  ...
  const save = () => {
    ...
    const promise = modal.open && modal.designation
      ? organizationApi.updateDesignation(modal.designation.id, { name: form.name.trim(), level: form.level, isLeader: form.isLeader, sortOrder: form.sortOrder })
      : organizationApi.createDesignation({ departmentId: deptId, name: form.name.trim(), level: form.level, isLeader: form.isLeader, sortOrder: form.sortOrder });
    promise
  ...
  const remove = (d: OrgDesignation) => {
    ...
    setSaving(true);
    organizationApi.deleteDesignation(d.id)
